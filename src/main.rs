...
use bamboo_mcp::core::component_registry::ComponentRegistry;
use bamboo_mcp::core::config_manager::load_config;
use bamboo_mcp::core::config_schema::Transport;
use bamboo_mcp::core::health_check_manager::HealthCheckManager;
use bamboo_mcp::core::logger::init_logging;
use bamboo_mcp::core::mcp_server::{McpifyServer, connect_stdio};
use bamboo_mcp::core::otel;
use bamboo_mcp::core::shutdown_handler::{install_shutdown_handlers, on_shutdown};
use bamboo_mcp::data::store::{open_store, resolve_store_path};
use bamboo_mcp::http::server::start_http_server;
use bamboo_mcp::http::types::HttpServerConfig;

#[derive(Parser)]
#[command(name = "bamboo-mcp", about = "Bamboo MCP server", version)]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
#[derive(Subcommand)]
enum Command {
    /// Interactively configure Bamboo credentials and connection settings
    Setup,
...
async fn run_harness_server() -> anyhow::Result<()> {
    ...
    let (otel_layer, otel_provider) = match otel::build_layer("bamboo-mcp") {
    ...
    if config.transport == Transport::Http {
        ...
        let (header_location, header_name) =
            bamboo_mcp::auth::auth_manager::header_location_for(config.auth_method);
...
async fn main() -> anyhow::Result<()> {
    ...
    let result = match cli.command {
        ...
        Command::Start => {
            ...
            unsafe {
                std::env::set_var("BAMBOO_MCP_TRANSPORT", "stdio");
            }
        ...
        Command::Http {
            ...
            // spawns any concurrent work that would also touch env vars.
            unsafe {
                std::env::set_var("BAMBOO_MCP_TRANSPORT", "http");
                if let Some(host) = &host {
                    std::env::set_var("BAMBOO_MCP_HOST", host);
                }
                if let Some(port) = port {
                    std::env::set_var("BAMBOO_MCP_PORT", port.to_string());
                }
                if let Some(cors_allow) = &cors_allow {
                    std::env::set_var("BAMBOO_MCP_CORS_ALLOW", cors_allow);
                }
            }
            run_harness_server().await
        }
