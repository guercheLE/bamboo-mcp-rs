...
const LOG_LEVEL_ENV_VAR: &str = "BAMBOO_MCP_LOG_LEVEL";
const TRANSPORT_ENV_VAR: &str = "BAMBOO_MCP_TRANSPORT";
...
/// `BAMBOO_MCP_LOG_LEVEL`, defaulting to `"info"` when unset.
pub fn resolve_log_level() -> String {
    resolve_log_level_from(std::env::var(LOG_LEVEL_ENV_VAR).ok())
}

/// `true` when the active transport is HTTP (see `main::run_harness_server`
/// / the `Command::Http` branch, which set `BAMBOO_MCP_TRANSPORT=http`
/// before this is read) — in that mode stdout carries no MCP protocol
/// traffic, so logs go there instead of stderr. See `init_logging`.
...
pub fn init_logging(otel_layer: Option<BoxedLayer>) {
    ...
    // (tracing's default) corrupts every frame parsed on the client side, so
    // logs go to stderr in that mode. In HTTP mode stdout carries no
    // protocol traffic, so logs go there instead, per convention. See
    // `logs_to_stdout`.
