...
impl McpifyServer {
    ...
    #[tool(description = "Semantic search for Bamboo operations using a natural-language query.")]
    async fn search(
        &self,
        ...
        self.run_tool("search", async move {
            // Recovers from poisoning instead of propagating it: this
            // `Mutex` is the single process-wide connection shared by
            // every session (see `cached_store_connection`'s doc comment),
            // so a panic in any one session's query would otherwise wedge
            // every other session's `search`/`get`/`call` for the rest of
            // the process's life. Every access here is read-only, so a
            // torn write from the panicking caller is never a concern.
    ...
    #[tool(
        description = "Return the schema, path, method, and documentation for a specific Bamboo operationId."
    )]
    ...
    #[tool(
        description = "Validate arguments, invoke a live Bamboo API operation, and validate the response."
    )]
    async fn call(
        &self,
...
// `new()` already built into this instance's `tool_router` field. Same
// reasoning applies to `#[prompt_handler(router = self.prompt_router.clone())]`
// below for `list_prompts`/`get_prompt`; the two handler macros stack on
// this one `impl ServerHandler` block because each only contributes its
// own disjoint set of methods.
...
mod tests {
    ...
    async fn search_and_get_return_mcp_content_envelopes() {
        ...
        let operation_id = {
            let conn = cached_store_connection("default").unwrap();
            let conn = conn
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner);
    ...
    // client awaiting a response that will never arrive) used to run
    // until the CI job's own 6-hour ceiling killed it. Failing fast here
    // instead turns that into a normal, fast test failure.
    #[tokio::test]
