...
const VERSION_STORE_FILES: &[(&str, &str)] = &[("default", "mcp_store.db")];
...
/// it, except `bamboo-mcp-populate-embeddings` (a separate `[[bin]]`), which uses
...
/// `bamboo-mcp-populate-embeddings` run, a deployment replacing the file) can update
