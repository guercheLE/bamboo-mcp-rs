...
fn generated_server(args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_bamboo-mcp"))
        .args(args)
...
fn version_help_versions_and_config_are_available() {
    ...
    assert!(stdout(&versions).contains("default"));
    ...
    assert_eq!(config["api_version"], "default");
...
fn search_and_get_exercise_the_embedded_operation_catalog() {
    ...
    let get = generated_server(&["get", "getBrokenBuildsForUser"]);
    assert!(get.status.success(), "{}", stderr(&get));
    let operation: serde_json::Value = serde_json::from_slice(&get.stdout).unwrap();
    assert_eq!(operation["operation_id"], "getBrokenBuildsForUser");
}
...
fn invalid_commands_operations_and_arguments_fail_without_network_calls() {
    ...
    let invalid_json = generated_server(&["call", "getBrokenBuildsForUser", "--args", "not-json"]);
...
fn stdio_and_helper_binaries_cover_their_bootstrap_and_error_paths() {
    ...
    let healthy = Command::new(env!("CARGO_BIN_EXE_bamboo-mcp-healthcheck"))
    ...
    let unhealthy = Command::new(env!("CARGO_BIN_EXE_bamboo-mcp-healthcheck"))
        .current_dir(empty.path())
        .output()
        .unwrap();
    assert!(!unhealthy.status.success());

    let populate = Command::new(env!("CARGO_BIN_EXE_bamboo-mcp-populate-embeddings"))
    ...
    .unwrap();
    let populated = Command::new(env!("CARGO_BIN_EXE_bamboo-mcp-populate-embeddings"))
        .arg(&store_copy)
        .output()
        .unwrap();
...
fn http_command_serves_health_and_shuts_down_cleanly() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_bamboo-mcp"))
        .args([
            "http",
            "--host",
            "127.0.0.1",
